...
public class NettyPacketHandler extends ChannelInboundHandlerAdapter {
	...
	public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
		...
		if (inboundPacket instanceof PacketHandshakeInHandshake handshake) {

			if (handshake.getNextState() == PacketHandshakeInHandshake.HandshakeState.LOGIN) {
				connection.setState(ConnectionState.LOGIN);
				System.out.println("--------------------------------\nSwitching state to LOGIN");
				System.out.println("--------------------------------");
			} else {
				connection.setState(ConnectionState.STATUS);
				System.out.println("--------------------------------\nSwitching state to STATUS");
				System.out.println("--------------------------------");
			}
		}

		else if (inboundPacket instanceof PacketStatusInStatusRequest statusRequest) {
			...
			description.addProperty("text", "§aHello, world!");
		...
		else if (inboundPacket instanceof PacketStatusInPingRequest pingRequest) {
			connection.sendPacket(new PacketStatusOutPingResponse(pingRequest.getPayload()));
		}

		else if (inboundPacket instanceof PacketLoginInLoginStart loginStart) {
			connection.sendPacket(new PacketLoginOutSuccess(loginStart.getUuid(), loginStart.getName()));
		}

		else if (inboundPacket instanceof PacketLoginInAcknowledge) {
			connection.setState(ConnectionState.CONFIGURATION);
			System.out.println("--------------------------------\nSwitching state to CONFIGURATION");
			System.out.println("--------------------------------");

			connection.sendPacket(new PacketConfigurationOutFinish());
		}

		else if (inboundPacket instanceof PacketConfigurationInAcknowledgeFinish) {
			connection.setState(ConnectionState.PLAY);
			System.out.println("--------------------------------\nSwitching state to PLAY");
			System.out.println("--------------------------------");
		}
	}
}
